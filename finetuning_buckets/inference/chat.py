...
class Chat:
    ...
    def __init__(self, model, prompt_style, tokenizer, max_length=2048, 
        ...
        if prompt_style == 'llama2':
            ...
            from finetuning_buckets.models.model_families.qwen import QwenStringConverter, default_system_prompt, qwen_stopping_criteria
            self.string_formatter  = QwenStringConverter.string_formatter_completion_only
            self.default_system_prompt = default_system_prompt
            self.stopping_criteria = qwen_stopping_criteria
        else:
    ...
    def generate_one_shot(self, input, max_new_tokens = 1024, 
        ...
        outputs = self.model.generate(
                input_ids = tokens_input,
                attention_mask = tokens_input.ne(self.tokenizer.pad_token_id), # attention mask for padding
                max_new_tokens=max_new_tokens,
                do_sample=do_sample,
                top_p=top_p,
                temperature=temperature,
